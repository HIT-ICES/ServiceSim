 ...
 * Creates a routing table considering the shortest path between devices.
 */
        ...
        for (int row = 0; row < size; row++) {
            for (int column = 0; column < size; column++) {
                ...
                if (routing.containsKey(sourceId)) {
                    ...
                    routingString.get(devices.get(row).getName()).put(devices.get(column).getName(), Objects.requireNonNull(getFogDeviceById(routingMatrix[row][column], devices)).getName());
                } else {
                    ...
                    routeS.put(devices.get(column).getName(), Objects.requireNonNull(getFogDeviceById(routingMatrix[row][column], devices)).getName());
