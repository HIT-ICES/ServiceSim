...
public class CompAllUpdateDevicesProvider1 extends DevicesProvider {
    ...
    public void createDevices() {
        ...
        for (NetworkDeviceAllVmUpdate smallBSDevice : smallBS_devices) {

            Channel channel = new Channel(smallBSDevice.getId(), router.get(0).getId(), 13 * 1024 * 1024, 0.0002, new PacketSchedulerTimeShared());
            smallBSDevice.getParentDeviceIds().add(router.get(0).getId());
            smallBSDevice.getParentDevicesToChannel().put(router.get(0).getId(), channel);

            Channel channel1 = new Channel(router.get(0).getId(), smallBSDevice.getId(), 13 * 1024 * 1024, 0.0002, new PacketSchedulerTimeShared());
            router.get(0).getChildDeviceIds().add(smallBSDevice.getId());
            router.get(0).getChildDevicesToChannel().put(smallBSDevice.getId(), channel1);
