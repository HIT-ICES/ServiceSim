...
//                    }
                    ...
                    Log.printLine(CloudSim.clock() + ": " + getName() + ": request packet #" + networkPacket.getAppId()
                            + " has arrival in Datacenter #" + getId() + " and was routed to vm #" + instanceId);
    ...
    public void networkPacketSend() {
        ...
        for (int key : sendToSameLevelDevicesPktList.keySet()) {
            ...
            if (nextCheckInterval < 0) {
                Log.printLine("NETWORK DEVICE ERROR: NextCheckInterval error.");
            }
        ...
        for (int key : sendToChildDevicesPktList.keySet()) {
            ...
            if (nextCheckInterval < 0) {
                System.out.println("NETWORK DEVICE ERROR: NextCheckInterval error.");
            }
        ...
        for (int key : sendToParentDevicesPktList.keySet()) {
            ...
            Log.printLine("parent channel:" + channel);
    ...
    public void cloudletSubmit() {
        ...
        for (int key : instanceIdToCloudlet.keySet()) {
            ...
            Log.printLine(CloudSim.clock() + ": " + getName() + ": some cloudlets"
                    + "has arrival in Datacenter #" + getId() + " and submitted to vm #" + vm.getId());
    ...
    public void cloudletProcessChecking(Vm vm) {
        ...
        if (getVmsNeedToDestroy().containsKey(vm.getId())) {
            if (networkCloudletScheduler.getCloudletExecList().isEmpty()) {
    ...
    public void processCloudletUpdateForResponse() {
        ...
        for (NetworkPacket networkPacket : getReceivedResponsePackets()) {
            networkPacket.setRecvTime(CloudSim.clock());
            networkPacketMap.computeIfAbsent(networkPacket.getDestinationVm(), k -> new ArrayList<>());
        ...
        for (int instanceId : networkPacketMap.keySet()) {
            ...
            for (NetworkPacket networkPacket : networkPacketMap.get(instanceId)) {
                scheduler.getPktRecv().computeIfAbsent(networkPacket.getDestinationCloudlet(), k -> new ArrayList<>());
    ...
    public void serviceRegistration(SimEvent ev) {
        ...
        MicroserviceInstance instance = data.getValue();
