 ...
 * be used, as processing of cloudlets is handled by the CloudletScheduler and processing of
 * VirtualMachines is handled by the VmAllocationPolicy.
 *
...
public class Datacenter extends SimEntity {
    ...
    protected void registerOtherEntity() {
        // Empty. This should be overridden by a child class
    }
    ...
    public void processEvent(SimEvent ev) {
        ...
        switch (ev.getTag()) {
            ...
            // Ping packet
            case CloudSimTags.INFO_PKT_SUBMIT:
    ...
    protected void processDataDelete(SimEvent ev, boolean ack) {
        ...
        } else { // if an error occurred, notify user
    ...
    protected void processDataAdd(SimEvent ev, boolean ack) {
        ...
        file.setMasterCopy(true); // set the file into a pass key
        ...
        if (ack) {
            ...
            data[2] = msg; // the result of adding a primary file
    ...
    protected void processPingRequest(SimEvent ev) {
        ...
        pkt.setTag(CloudSimTags.INFO_PKT_RETURN);
        pkt.setDestId(pkt.getSrcId());

        // sends back to the sender
        sendNow(pkt.getSrcId(), CloudSimTags.INFO_PKT_RETURN, pkt);
    }

    /**
     * Process the event for a User/Broker who wants to know the status of a Cloudlet.
     ...
     * PowerDatacenter.
     * This PowerDatacenter may send, upon request, the status back to the User/Broker.
     *
     * @param ev  a Sim_event object
     * @param ack the ack
     * @pre ev != null
    ...
    /**
     * Process the event for a User/Broker who wants to migrate a VM.
     * This PowerDatacenter will then send the status back to the User/Broker.
     *
     * @param ev a Sim_event object
     * @pre ev != null
    ...
    protected void processCloudletSubmit(SimEvent ev, boolean ack) {
        ...
        try {
            ...
            if (estimatedFinishTime > 0.0 && !Double.isInfinite(estimatedFinishTime)) {
                ...
                // ESTIMATED FINISH TIME ON "+((StreamOperator)vm).getName()+":
                // "+estimatedFinishTime);
            ...
            logger.error("Error in processing cloudlet submit", c);
        } catch (Exception e) {
            Log.printLine(getName() + ".processCloudletSubmit(): " + "Exception error.");
            logger.error("Error in processing cloudlet submit", e);
        }
     ...
     * It is necessary because Hosts and VirtualMachines are simple objects, not entities.
     * So, they don't receive events and updating cloudlets inside them must be called from the outside.
     *
     * @pre $none
     * @post $none
     */
     ...
     * If the file is a primary file, then it will be registered to the RC when the experiment begins.
     *
