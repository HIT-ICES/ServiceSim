...
public class DatacenterCharacteristics {
    ...
    /**
     * Spaced-shared system using First Come, First Served (FCFS) algorithm.
     */
    ...
    /**
     * Price/CPU-unit of unit = sec., then G$/CPU-sec.
     */
    ...
    /**
     * Gets a Machine with at least a given number of free pes.
     *
     ...
     * It is assumed all PEs' rating is the same in a given machine.
     *
    ...
    /**
     * Gets Millions Instructions Per Second (MIPS) Rating of a Processing Element (Pe).
     * It is essential to use this method when a resource is made up of heterogeneous PEs/machines.
     *
    ...
    public int getMipsOfOnePe(int id, int peId) {
        ...
        return PeList.getMips(((Host) Objects.requireNonNull(HostList.getById(getHostList(), id))).getPeList(), peId);
    }

    /**
     * Gets the total MIPS rating, which is the sum of MIPS rating in all machines in a resource.
     * <p>
     * Total MIPS rating for:
     * <ul>
     * <li>TimeShared = 1 Rating of a Pe * Total number of PEs
     * <li>Another policy same rating = same as TimeShared
     * <li>SpaceShared = Sum of all PEs in all Machines
     * <li>Other policy different rating = same as SpaceShared
     * <li>Reservation = 0 or unknown. You need to calculate this manually.
     * </ul>
     *
    ...
    public int getMips() {
        ...
        switch (getAllocationPolicy()) {
            // Assuming all PEs in all Machine have the same rating.
     ...
     * <tt>NOTE:</tt> The CPU time for SPACE_SHARED and ADVANCE_RESERVATION are not yet implemented.
     *
     ...
     * @param peId   Pe id
     * @return otherwise (Machine id or Pe id might not be existed)
     * @pre machineID >= 0
     * @pre peID >= 0
     * @post $none
     */
     ...
     *
     * @return the current number of failed machines this resource has.
     */
