...
public class ServiceProvider extends DatacenterBroker {
    ...
    // failed created vms
    private final String appCloudletResultFile;
    private final String cloudletExeDetailFile;
    private final String cloudletStageDetailFile;

    // record
    private final Map<Integer, ArrayList<NetworkPacket>> endUserRequest;
...
//        }
        ...
        new NetworkCloudletTimeSharedScheduler();
...
//        }
    ...
    protected void processVmCreate(SimEvent ev) {
        ...
        if (result == CloudSimTags.TRUE) {
            ...
            Log.printLine(CloudSim.clock() + ": " + getName() + ": VM #" + vmId
                    ...
                    + ((Vm) Objects.requireNonNull(VmList.getById(getVmsCreatedList(), vmId))).getHost().getId());
        } else {
    ...
    public void processVmDestroy(SimEvent ev) {
        ...
        if (result == CloudSimTags.TRUE) {
            ...
            Log.printLine(CloudSim.clock() + ": " + getName() + ": VM #" + vmId
                    ...
                    + ((Vm) Objects.requireNonNull(VmList.getById(getVmsDestroyedList(), vmId))).getHost().getId());
        } else {
    ...
    protected void processCloudletReturn(SimEvent ev) {
        ...
        NetworkPacket networkPacket = data.getValue();
        NetworkCloudlet cloudlet = data.getKey();
        ...
        if (vm != null && vm.getServiceId() == 0) {// gateway
    ...
    protected void writeNetworkCloudletResult(String fileName, NetworkCloudlet cloudlet, MicroserviceInstance vm, NetworkPacket networkPacket) {
        ...
        try {
            ...
            logger.error("Error when closing the file writer", e);
        }
    }
    ...
    protected void writeAppCloudletResult(String fileName, NetworkCloudlet cloudlet, MicroserviceInstance ignoredVm, NetworkPacket networkPacket) {
        FileWriter fileWriter = null;
        ...
        try {
            ...
            logger.error("Error when closing the file writer");
        }
    }
