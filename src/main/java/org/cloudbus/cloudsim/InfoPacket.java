 ...
 * networks.
 * <p>
 * You can set all the parameters to an InfoPacket that can be applied to a NetPacket.
 * To effectively gather information on the network experience of a specific type in NetPacket, you should mirror its
 * attributes in InfoPacket.
 * This involves configuring the InfoPacket to have the same size and network class as the NetPacket you're analyzing.
 * Then, ensure that the InfoPacket is sent from the identical source and directed to the same destination as the
 * original NetPacket.
 * By doing so,
 * you can simulate the same network conditions and collect relevant data on how the NetPacket is processed
...
public class InfoPacket implements Packet {
    ...
    private void init() {
        last = srcId;
        tag = CloudSimTags.INFO_PKT_SUBMIT;
        bandwidth = -1;
        hopsNumber = 0;
        pingSize = size;
    ...
    /**
     * Returns human-readable information of this packet.
     *
     ...
     * CloudResources.
     * It should not be called by links, etc.
     *
     ...
     *
     * @param time current simulation time, use {@link gridsim.CloudSim#clock()} to get this
     * @pre time >= 0
     * @post $none
     */
     ...
     * them.
     * It should also supply the current simulation time.
     *
     * @param time current simulation time, use {@link gridsim.CloudSim#clock()} to get this
     * @pre time >= 0
     * @post $none
	...
	public boolean setTag(int tag) {
		...
		switch (tag) {
			...
			case CloudSimTags.INFO_PKT_RETURN:
				this.tag = tag;
				flag = true;
				break;
