 ...
 * network. Later nodes in such a file are mapped to CloudSim entities. Delay calculated from the
 * BRITE model is added to the messages sent through CloudSim. Messages using the old model are
 ...
 */
@SuppressWarnings({"CommentedOutCode", "unused", "UnusedReturnValue"})
public class CloudSim {
    private static final Logger logger = LoggerFactory.getLogger(CloudSim.class);
...
//    end
     ...
     *                  These parameters indicate that{@link gridsim.CloudSimShutdown} first waits
     *                  for all user entities' END_OF_SIMULATION signal before issuing terminate
     *                  signal to other entities
     * @param cal       starting time for this simulation.
     *                  If it is <tt>null</tt>, then the time will be taken from
     *                  <tt>Calendar.getInstance()</tt>
     * @param traceFlag <tt>true</tt> if CloudSim trace needs to be written
     * @pre numUser >= 0
     * @post $none
     ...
     *
     * @param numUser             the number of User Entities created.
     *                            These parameters indicate that {@link gridsim.CloudSimShutdown}
     *                            first waits for all user entities' END_OF_SIMULATION signal before
     *                            issuing terminate signal to other entities
     * @param cal                 starting time for this simulation.
     *                            If it is <tt>null</tt>, then the time will be taken from
     *                            <tt>Calendar.getInstance()</tt>
     * @param traceFlag           <tt>true</tt> if CloudSim trace needs to be written
     * @param periodBetweenEvents - The minimal period between events.
     ...
     * It waits for complete execution of all entities, i.e.,
     * until all entity threads reach non-RUNNABLE state or there are no more events in the future event queue.
     * <p>
     * <b>Note</b>: This method should be called after all the entities have been set up and added.
     *
     * @return the double
    ...
    public static double startSimulation() throws NullPointerException {
        ...
        try {
            ...
            logger.error("Error in CloudSim.startCloudSimulation()", e);
    ...
    /**
     * Stops Cloud Simulation (based on {@link Simulation#runStop()}).
     * This should be only called if any of the user defined entities <b>explicitly</b> want to terminate simulation
     * during execution.
     *
     * @throws NullPointerException This happens when creating this entity before initialising
     *                              CloudSim package or this entity name is <tt>null</tt> or empty
     ...
     *
     * @return a new copy of a Calendar object or if CloudSim hasn't been initialized
     * @pre $none
     * @post $none
     ...
     * This function should be called at the start of the simulation.
     */
     ...
     * This is present for compatibility with existing simulations since entities are automatically added to the
     ...
     * It should <b>not</b> be called from user simulations.
     *
     * @param e The new entity
     */
     ...
     * This method should <b>not</b> be called in simulations.
     *
     * @return true, if successful otherwise
     */
    public static boolean runClockTick() {
        ...
        if (future.size() > 0) {
            ...
            // Check if next events are at the same time...
...
//				SimEvent next = fit.next();
//				if (next.eventTime() == first.eventTime()) {
//					// my: Judge whether it is the same event
//					// my: The same event is not processed twice at the same time
//					if (next.getDestination() == first.getDestination() && next.getTag() == first.getTag() && next.getData() == first.getData()){
//						toRemove.add(next);
//						tryMore = fit.hasNext();
//						continue;
//					}
...
//                e.CustomString(String.valueOf(++eCounter)+": Cloudsim(future queue event added)\n");
        ////////////////////////////////////////////////////////////////////////////////////////////
        future.addEvent(e);
    }
     ...
     * The predicate used to wait for an event is now passed to Sim_system.
     * Only events that satisfy the predicate will be passed to the entity.
     * This is done to avoid unnecessary context switches.
     *
     * @param src the src
     * @param p   the p
     */
    public static void wait(int src, Predicate p) {
        ...
        if (p != SIM_ANY) {
            // If a predicate has been used, store it to check it
     ...
     * at least one event has been canceled; false otherwise.
     *
     * @param src the src
     * @param p   the p
    ...
    private static void processEvent(SimEvent e) {
        ...
        switch (e.getType()) {
            ...
            case SimEvent.CREATE:
                SimEntity newer = (SimEntity) e.getData();
                addEntityDynamically(newer);
                break;
     ...
     * This method should <b>not</b> be used by user simulations.
     */
     ...
     * This should be called after all the entities have been set up and added, and their ports linked.
     *
        ...
        while (true) {
            ...
            while (paused) {
                try {
                    ...
                    logger.error("Error in CloudSim.run()", e);
                }
            }
        }
     ...
     * This method should <b>not</b> be used in user simulations.
     */
