 ...
 * Tarting ezra bar assess 1- clock ezra 2- Identity 3- serial
 *
...
public class SimEvent implements Cloneable, Comparable<SimEvent> {
    ...
    /**
     * the user defined a type of the event
     **/
    ...
    SimEvent(int etype, double time, int src, int dest, int tag, Object edata) {
        this.etype = etype;
        this.time = time;
        entSrc = src;
        entDst = dest;
    ...
    SimEvent(int etype, double time, int src) {
        this.etype = etype;
        this.time = time;
        entSrc = src;
        entDst = -1;
     ...
     *
     * @param end_waiting_time the time at which this event finished waiting
     */
