 ...
 * It simulates the behavior of a typical hard drive storage.
 * The default values for this storage are those of a Maxtor DiamondMax 10 ATA hard disk with the following parameters:
 * <ul>
 * <li>latency = 4.17 ms
...
public class HarddriveStorage implements Storage {
    ...
    /**
     * a list storing the names of all the files on the hard drive.
     */
    private List<String> nameList;
    /**
     * a list storing all the files stored on the hard drive.
     */
    ...
    /**
     * The initialization of the hard drive is done in this method.
     * The most common parameters, such as latency, averages seek time and maximum transfer rate are set.
     ...
     * @return the time (in seconds) required adding the file
     */
    @Override
    public double addReservedFile(File file) {
        ...
        // if an added file fails, then set the current size back to its old value
        if (result == 0.0) {
            currentSize += file.getSize();
        }
    ...
    public boolean hasPotentialAvailableSpace(int fileSize) {
        ...
        // if not enough space, then if you want to clear/delete some files,
        // then check whether it still has space or not
     ...
     * The time taken (in seconds) for getting the file can also be found using
     * {@link gridsim.datagrid.File#getTransactionTime()}.
     *
     * @param fileName the name of the necessary file
     * @return the file with the specified filename
     */
    @Override
    public File getFile(String fileName) {
        // check first whether the file name is valid or not
        File obj = null;
     ...
     * Given a file size in MB, this method returns a seek time for the file in seconds.
     *
     ...
     * This method checks whether the given file or the file name of the file is valid.
     * The method name parameter is used for debugging purposes, to output in which method an error has occurred.
     *
     * @param file       the file to be checked for validity
     * @param methodName the name of the method in which we check for the validity of the file
     * @return <tt>true</tt> if the file is valid, <tt>false</tt> otherwise
     */
    private boolean isFileNotValid(File file, String methodName) {
     ...
     * First, the method checks if there is enough space on the storage, then it checks if the file with the same name
     * is already taken
     * to avoid duplicate filenames. <br> The time taken (in seconds) for adding the file can also be found using
     * {@link gridsim.datagrid.File#getTransactionTime()}.
     *
     * @param file the file to be added
    ...
    public double addFile(File file) {
        double result = 0.0;
        // check if the file is valid or not
        if (isFileNotValid(file, "addFile()")) {
            return result;
        }
     ...
     * The time taken (in seconds) for adding each file can also be found using
     * {@link gridsim.datagrid.File#getTransactionTime()}.
     *
     ...
     * The time taken (in seconds) for deleting the file can also be found using
     * {@link gridsim.datagrid.File#getTransactionTime()}.
     *
    ...
    public double deleteFile(File file) {
        double result = 0.0;
        // check if the file is valid or not
        if (isFileNotValid(file, "deleteFile()")) {
            return result;
        }
        double seekTime = getSeekTime(file.getSize());
        double transferTime = getTransferTime(file.getSize());
        ...
        if (contains(file)) {
            ...
            nameList.remove(file.getName());  // remove the name from a name list
    ...
    public boolean contains(File file) {
        boolean result;
        if (isFileNotValid(file, "contains()")) {
            return false;
        }
     ...
     * The time taken (in seconds) for renaming the file can also be found using
     * {@link gridsim.datagrid.File#getTransactionTime()}.
     *
