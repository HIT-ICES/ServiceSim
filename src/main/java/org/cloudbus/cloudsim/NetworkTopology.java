 ...
 * <p>
 * The topology file may contain more nodes the number of entities in the simulation.
 * It allows for users to increase the scale of the simulation without changing the topology file.
...
public class NetworkTopology {
    ...
    /**
     * Creates the network topology if a file exists and if a file can be successfully parsed.
     * The File is written in the BRITE format and contains topological information on simulation entities.
     *
    ...
    /**
     * Creates the matrix containing the available bandwidth between two nodes
     *
     * @param graph topological graph describing the topology
     * @return the bandwidth graph
     */
    private static double[][] createBwMatrix(TopologicalGraph graph) {
    ...
    public static double getDelay(int srcID, int destID) {
        if (networkEnabled) {
            try {
                ...
                // in case of error, keep running and return 0.0
            }
        }
        return 0.0;
    }
     ...
     * If there were some problem during creation of network (e.g., during parsing of a BRITE file) that does not allow
